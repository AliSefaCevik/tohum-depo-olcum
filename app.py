...
st.write("Yarıda kalan veya hatalı tüm depo ölçümlerini tek tıkla silebilirsiniz.")
...
if st.button("🔴 TÜM FABRİKA VERİLERİNİ SIFIRLA", use_container_width=True):
    if onay_kutusu:
        st.session_state.fabrika_verisi = {f"Depo {i}": {} for i in range(1, 13)}
        st.warning("🔄 Bütün depoların hafızası tamamen sıfırlandı! Yeni tura başlayabilirsiniz.")
        st.rerun()
    else:
        st.error("Önce yukarıdaki onay kutusunu işaretlemelisiniz!")
...
if st.button("💾 TÜM DEPOLARI EXCEL SEKMELERİNE AKTAR VE İNDİR", use_container_width=True):
    ...
    if aktif_depo_sayisi == 0:
        st.error("En az 1 deponun tüm ölçümlerini tam doldurup hafızaya kaydetmelisin!")
    else:
